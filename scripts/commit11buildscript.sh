#!/bin/bash
# <!-- WSL: Автоматическая конвертация путей для Windows -->
# Создание парсера артефактов с WSL-спецификой

echo -e "🚀 Создание файла src/parser_logic.py..."
...
class ArtifactParser:
    ...
    def is_valid_artifact(artifact_text: str) -> bool:
        ...
        try:
            ...
            if not re.search(r'---\s*ARTIFACT_START\s*---', artifact_text):
                return False
...
echo -e "✅ Парсер создан в $SRC_PATH/parser_logic.py"
```

### 3. Дополнительные проверки качества
#### ✨ Автоматическая проверка в CI/CD
Добавлена проверка для соответствия YFM и структуре:
...
#### ✨ Расширенные тесты для парсера
Добавлены тесты с использованием pytest:
...
def test_parse_valid_meta():
    ...
    text = """--- ARTIFACT_START ---
---META:project=SLOS;type=test;name=valid;version=v1.0.0;status=production---
## Overview
Test artifact
--- ARTIFACT_END ---"""
...
def test_parse_missing_required_field():
    ...
    text = """--- ARTIFACT_START ---
---META:project=SLOS;type=test;name=invalid;status=production---
## Overview
Test artifact
--- ARTIFACT_END ---"""
...
def test_parse_invalid_version():
    ...
    text = """--- ARTIFACT_START ---
---META:project=SLOS;type=test;name=invalid;version=1.0.0;status=production---
## Overview
Test artifact
--- ARTIFACT_END ---"""
...
---META:project=SLOS;type=test;name=yfm;version=v1.0.0;status=production---
## Overview
::: python
print("Hello YFM")
:::
...
---META:project=SLOS;type=test;name=non-yfm;version=v1.0.0;status=production---
## Overview
```python
...
## Examples
### Полная реализация парсера (WSL/Bash)
```bash
#!/bin/bash
# <!-- WSL: Автоматическая конвертация путей для Windows -->
# Скрипт для создания парсера артефактов с WSL-спецификой
...
if ! pip show pytest > /dev/null 2>&1; then
    echo -e "${YELLOW}Установка pytest...${NC}"
    pip install pytest
fi
